...
class radarObject:
	...
	global shownOff
	"""
	def __init__ (self,ID,latX,longY,shownOff,sector):
		self.ID = ID
	...
	def parseMessgageToArrayMR72 (self,message):
		...
		self.shownOff = 0
		self.sector = ((message.data[6] >> 3) & 0X03)
...
for i in range(256):
	arrayMR72.append(radarObject(0,0,0,0,0))
	arrayMR72_1.append(radarObject(0,0,0,0,0))
	arrayMR72_2.append(radarObject(0,0,0,0,0))
	arrayMR72_3.append(radarObject(0,0,0,0,0))
...
def sendToUDP(arraryMR72, sendWhatMR72):
	...
	if abs(timeNow - timeBefore) > 100:
		...
		if sendWhatMR72 == 0:
			strAll = 'front,'
			for i in range(4):
				if arrayMR72[i].sector == 1:
					mag = math.sqrt(pow(arrayMR72[i].latX,2) + pow(arrayMR72[i].longY,2))
					sectors[i-1] = mag
					arrayMR72[i].sector = 0
			
				
		elif sendWhatMR72 == 1:
			strAll = 'right,'
			for i in range(4):
				if arrayMR72_1[i].sector == 1:
					mag = math.sqrt(pow(arrayMR72_1[i].latX,2) + pow(arrayMR72_1[i].longY,2))
					sectors[i-1] = mag
					arrayMR72_1[i].sector = 0
				
		elif sendWhatMR72 == 2:
			strAll = 'rear,'
			for i in range(4):
				if arrayMR72_2[i].sector == 1:
					mag = math.sqrt(pow(arrayMR72_2[i].latX,2) + pow(arrayMR72_2[i].longY,2))
					sectors[i-1] = mag
					arrayMR72_2[i].sector = 0
			#print sectors
		elif sendWhatMR72 == 3:
			strAll = 'left,'
			for i in range(4):
				if arrayMR72_3[i].sector == 1:
					mag = math.sqrt(pow(arrayMR72_3[i].latX,2) + pow(arrayMR72_3[i].longY,2))
					sectors[i-1] = mag
					arrayMR72_3[i].sector = 0
		...
		if message.arbitration_id == 0X60A or message.arbitration_id == 0X61A or message.arbitration_id == 0X62A or message.arbitration_id == 0X63A:
			...
			if (message.data[0] == 0X01) and (message.data[2] == 0x00) and(message.data[3] == 0x00) and (message.data[4] == 0x00) and (message.data[5] == 0x00) and	(message.data[6] == 0x00) and (message.data[7] == 0x00):
				...
				if message.arbitration_id == 0X60A:
					...
					#print arrayMR72[0], arrayMR72[1], arrayMR72[2]
					#print '0'
			...
			beginToProcessRadar = False
			abc = radarObject(0,0,0,0,0)
			abc.parseMessgageToArrayMR72(message)
			#print abc.sector, abc.latX, abc.longY
			if abc.latX >= 0.1 or abc.longY >= 0.1:
				
				arrayMR72[abc.sector].sector = 1
				arrayMR72[abc.sector].latX = abc.latX
				arrayMR72[abc.sector].longY = abc.longY
			...
			if abc.latX >= 0.1 or abc.longY >= 0.1:
				...
				arrayMR72_1[abc.sector].sector = 1
				arrayMR72_1[abc.sector].latX = abc.latX
				arrayMR72_1[abc.sector].longY = abc.longY
			...
			if abc.latX >= 0.1 or abc.longY >= 0.1:
				...
				arrayMR72_2[abc.sector].sector = 1
				arrayMR72_2[abc.sector].latX = abc.latX
				arrayMR72_2[abc.sector].longY = abc.longY
			...
			if abc.latX >= 0.1 or abc.longY >= 0.1:
				...
				arrayMR72_3[abc.sector].sector = 1
				arrayMR72_3[abc.sector].latX = abc.latX
				arrayMR72_3[abc.sector].longY = abc.longY
