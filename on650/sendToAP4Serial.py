...
maxDist = 9999
distanceToStopinAuto = 800 #in cm
    ...
    if abc[0] == 'E' and abc[1] == ' nomr72':
        ...
        if abc[0] == 'front':
           # abcc.reverse() # quay lai, nguoc voi chieu default
            ...
            partFront = np.pad(partFront, (0, 248), 'constant',  constant_values=maxDist+1)
        ...
        if abc[0] == 'left':
           # abcc.reverse()
            ...
            partLeft = np.pad(partLeft, (0, 248), 'constant',  constant_values=maxDist+1)
            partLeft = np.roll(partLeft, 123)
            
        if abc[0] == 'rear':
           # abcc.reverse()
            partRear = np.asarray(abcc)            
            partRear = np.pad(partRear, (0, 248), 'constant',  constant_values=maxDist+1)
            partRear = np.roll(partRear, 213)
        if abc[0] == 'right':
           # abcc.reverse()
            partRight = np.asarray(abcc)
            partRight = np.pad(partRight, (0, 248), 'constant',  constant_values=maxDist+1)
        ...
        #print min8Array[0], min8Array[1]
        
        #min8ArrayInter[1] = min(obstaclesAround[22:67])
        #min8ArrayInter[0] = min(obstaclesAround[67:112])
        #min8ArrayInter[7] = min(obstaclesAround[112:157])
        #min8ArrayInter[6] = min(obstaclesAround[157:202])
        #min8ArrayInter[5] = min(obstaclesAround[202:247])
        #min8ArrayInter[4] = min(obstaclesAround[247:292])
        #min8ArrayInter[3] = min(obstaclesAround[292:337])
        #min8ArrayInter[2] = min(min(obstaclesAround[337:360]), min(obstaclesAround[0:22]))
            ...
            if rcAvoid > 1700:
                ...
                if rcHoriMag >= 0.05:# filtering magitude of RC stick
                    ...
                    if obstaclesAroundQuanti[rcHoriAngle] < distanceToWarn:
                        ...
                        ratioToRc = float((obstaclesAroundQuanti[rcHoriAngle] - distanceToStopinManual) / (distanceToWarn - distanceToStopinManual))
                        print (obstaclesAroundQuanti[rcHoriAngle])
                        print (ratioToRc)
                        if ratioToRc <= 0.00:
                            ratioToRc = 0
...
class sendMin (threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            singleThreadSendMinUsingSerial()
...
data[0] = 84
data[1] = 72
def singleThreadSendMinUsingSerial():
    for i in range(8):
        inte = int(min8Array[i]) * 10
        data[(i+1)*2] = inte >> 8 & 0XFF
        data[(i+1)*2 + 1] = inte & 0XFF
