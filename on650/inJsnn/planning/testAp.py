...
maxDist = 9999
distanceToStopinAuto = 3100 #in cm
distanceToStopinManual = distanceToStopinAuto
distanceToWarn = 5000
distanceToWarninAuto = 10000
obstaclesAround = []
localSecsNum = 9
angleToNextWP = 0
...
def parseSec(setors):
	...
	if inAuto or inRTL or inGuided:
		if rcAvoid > 1700:
			if missionList[currentMission,4] == 16:
				angleToNextWP = latLongReturnAngle(currLat, currLon, float(missionList[currentMission,11]) / 7 , float(missionList[currentMission,12]) / 7)
				print 'wp', currentMission  ,'angle to next WP', angleToNextWP	
			fooInd = int(angleToNextWP - ahrs2Yaw) + 90
			if fooInd >= 360:
				fooInd = fooInd - 360
			elif fooInd < 0:
				fooInd = fooInd + 360
			if obstaclesAround[fooInd] < distanceToWarninAuto:
				ratioToRc = float(obstaclesAround[fooInd] - distanceToStopinAuto) / float((distanceToWarninAuto - distanceToStopinAuto))
